...
| 2026-07-23 | Conversation History persistence ([ADR-0017](0017-history-is-an-immutable-versioned-store.md)) is an at-least-once effect under this ADR: History is saved before each commit, outside the atomic `Apply`, and a replayed or saved-but-not-committed attempt may re-save, so History can carry a duplicated turn. No journal, no exactly-once — the same posture as tool calls. |
| 2026-08-01 | ADR-0017 now stores History as immutable versions named by a ref that commits with State, so a saved-but-not-committed attempt leaves an unreferenced version rather than a duplicated turn. This ADR is unchanged: the save is still an at-least-once effect (a replay re-saves), and a replayed transition still re-calls the LLM and re-runs its tools. What rolls back is the conversation record, not the effects. |
