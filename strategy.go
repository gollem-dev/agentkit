...
type Strategy[S, I, O any] interface {
	...
	// Limiter returns this Process's budget policy, or nil for no budget. The
	// kernel measures (Metrics); the returned function decides (ADR-0010). It is
	// called exactly once, by BindStrategy at registration time, and the function
	// it returns runs at each transition boundary, before every Generate, CallTool
	// and SpawnChild, and again after each of those has been metered.
	//
	// Handing out a function rather than answering directly is what keeps the
	// decision independent of the strategy value: whatever the policy needs —
	// thresholds, a rate limiter, a table of per-tenant budgets — is captured once
	// when the function is built, instead of being re-derived on every one of the
	// 1 + 2×effects calls. The returned function must be cheap, read-only and
	// non-blocking; see the Limiter type for what that rules out.
	//
	// It takes no S and no Process: the boundary evaluation happens before the
	// state is decoded, and a limit that has to read the algorithm's own state is a
	// branch in Step, not a budget. The Process reaches the policy as the
	// function's own argument.
	//
	// Returning nil means unlimited, and the kernel then calls nothing at all.
	// There is no way to opt out of answering, which is the point: a budget nobody
	// configured used to read as no budget at all.
	Limiter() Limiter
...
type StrategyBinding struct {
	...
	// limit is what the strategy's Limiter method returned, taken once at
	// registration. It touches none of S, I or O, so it needs no erasing closure.
	// nil means the strategy said it has no budget, and every call site skips the
	// evaluation entirely rather than calling something that always passes.
	limit  Limiter
...
// was registered. Limiter is called here, exactly once, and its result stored as
// it is: it has no typed argument to erase. Calling it once is the contract — a
// strategy cannot hand out a different policy per claim, which is what keeps the
// decision independent of the strategy value. A panic in Limiter is the caller's,
// raised synchronously inside Register, so it is deliberately not recovered:
// callLimit exists for the boundary evaluation, which runs on a worker.
// Exported for building fake strategies in tests.
func BindStrategy[S, I, O any](s Strategy[S, I, O], opts ...RegisterOption[O]) StrategyBinding {
	...
	b.historyStore = cfg.historyStore
	return b
}
