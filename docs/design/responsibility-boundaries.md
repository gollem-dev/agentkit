...
| Cost limits | **strategy author** (`Strategy.Limiter`) | pricing is not kernel knowledge |
...
different: they receive a live `*Process`, and for a `Limiter` that is the very
...
- **Keep your `Limiter` cheap.** It runs before and after every effect and at
  every transition boundary. Derive whatever it needs when you build the closure,
  not on each call.
